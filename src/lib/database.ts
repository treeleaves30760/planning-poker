...
export class DatabaseManager {
  ...
  createGame(game: Game): boolean {
    ...
    try {
      stmt.run(
        ...
        game.isActive ? 1 : 0,
        game.createdAt instanceof Date ? game.createdAt.toISOString() : new Date(game.createdAt).toISOString()
      );
      return true;
    } catch (error) {
  ...
  getGame(gameId: string): Game | null {
    ...
    const game: Game = {
      ...
      currentTask: gameRow.current_task ? JSON.parse(gameRow.current_task) : null,
  ...
  updateGame(game: Game): boolean {
    const transaction = this.db.transaction(() => {
      ...
      const upsertTaskStmt = this.db.prepare(`
        ...
        (id, game_id, description, votes, revealed, allow_changes, completed_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `);
  ...
  updateUserHeartbeat(userId: string, gameId: string, username: string, isAdmin: boolean = false): boolean {
    const stmt = this.db.prepare(`
  ...
  removeUser(userId: string, gameId: string): boolean {
    const stmt = this.db.prepare('DELETE FROM users WHERE id = ? AND game_id = ?');
