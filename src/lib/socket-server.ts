...
class SocketManager {
	...
	private setupEventHandlers() {
		...
		this.io.on("connection", (socket) => {
			...
			socket.on(
				"leave-game",
				async (data: { gameId: string; userId: string }) => {
					try {
			...
			socket.on("disconnect", async (reason) => {
				...
				const userData = (socket as unknown as { userData?: UserData })
					.userData;
				if (userData) {
					try {
						...
						// Get updated active players count (excluding admins)
						const activePlayersCount = dbManager.getActivePlayersCount(
							userData.gameId
						);
						...
						console.log(
							`User ${userData.username} disconnected from game ${userData.gameId}`
						);
					} catch (error) {
		...
		this.httpServer
			...
			.on("error", (error: Error) => {
				if ("code" in error && error.code === "EADDRINUSE") {
					console.log(
						`Port ${port} already in use - Socket.io server may already be running`
					);
				} else {
